<script>
	...
	$(document).ready(function(){
		...
		function mostrarDatos(){
			$.ajax({
				type: 'ajax',
				url: '<?= base_url('notificacionController/get_notificacion') ?>',
				dataType: 'json',
				...
				success: function(datos){
					...
					for(i=0; i<datos.length; i++){
						...
						'<td>'+'<a href="javascript:;" class="edit" data="'+datos[i].id_notificacion+'" > <button data-toggle="tooltip" title="Edit" class="pd-setting-ed"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a>  '+
						'<td>'+'<a href="javascript:;" class="borrar" data="'+datos[i].id_notificacion+'" > <button data-toggle="tooltip" title="Trash" class="pd-setting-ed"><i class="fa fa-trash-o" aria-hidden="true" ></i></button></a>'+
						'</tr>';
						n++;
        ...
        $('#modalAgregar').find('.modal-title').text('Nueva notificacion');
        //modificamos el atributo action, le agregamos la ruta del controlador y modelo para ingresar
        $('#formulario').attr('action','<?= base_url('notificacionController/ingresar')?>');   
        });
        ...
        function get_estudiante(){
            $.ajax({
                type: 'ajax',
                url: '<?= base_url('notificacionController/get_estudiante') ?>',
                dataType: 'json',
        ...
        function get_doctor(){
            $.ajax({
                type: 'ajax',
                url: '<?= base_url('notificacionController/get_doctor') ?>',
                dataType: 'json',
        ...
        // Funcion ajax para el proceso de peticion de los datos de sintomas
        get_usuario();//llamado a la funcion

        function get_usuario(){
            $.ajax({
                type: 'ajax',
                url: '<?= base_url('notificacionController/get_usuario') ?>',
                dataType: 'json',
...
// Funcion ajax para hacer el proceso de edicion de datos
        $('#tabla_datos').on('click', '.edit', function(){
            ...
            $('#modalAgregar').modal('show');
            $('#modalAgregar').find('.modal-title').text('Editar notificacion');
            $('#formulario').attr('action', '<?= base_url('notificacionController/actualizar') ?>');
            $.ajax({
                type: 'ajax',
                method: 'post',
                url: '<?= base_url('notificacionController/get_datos') ?>',
                data: {id:id},
                dataType: 'json',
                success: function(datos){
                    $('#id').val(datos.id_notificacion);
                    $('#estudiante').val(datos.id_datos);
                    $('#usuario').val(datos.user2);
                    $('#celular_emerg').val(datos.celular_emerg);
                    $('#comentario').val(datos.comentario);
                    $('#detalle_not').val(datos.detalle_not);
                    $('#unnamed2').val(datos.unnamed2);
                    $('#grado_incidente').val(datos.grado_incidente);
        ...
        $('#tabla_datos').on('click', '.borrar', function(){
            ...
            $('#btnBorrar').unbind().click(function(){
                $.ajax({
                    type: 'ajax',
                    method: 'post',
                    url: '<?= base_url('notificacionController/eliminar') ?>',
                    data: {id:$id},
                    dataType: 'json',
                    success: function(respuesta){
