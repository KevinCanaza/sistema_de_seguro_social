<script>
	...
	$(document).ready(function(){
		...
		function mostrarDatos(){
			$.ajax({
				type: 'ajax',
				url: '<?= base_url('especialidadController/get_especialidad') ?>',
				dataType: 'json',
				...
				success: function(datos){
					...
					for(i=0; i<datos.length; i++){
                        ...
                        '<td>'+datos[i].tipo_especialidad+'</td>'+
						'<td> <button class="pd-setting">Active</button></td>'+
						'<td>'+'<a href="javascript:;" class="edit" data="'+datos[i].id_especialidades+'" > <button data-toggle="tooltip" title="Edit" class="pd-setting-ed"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a>  '+
						'<td>'+'<a href="javascript:;" class="borrar" data="'+datos[i].id_especialidades+'" > <button data-toggle="tooltip" title="Trash" class="pd-setting-ed"><i class="fa fa-trash-o" aria-hidden="true" ></i></button></a>'+
        ...
        $('#modalAgregar').find('.modal-title').text('Nuevo Especialidad');
        //modificamos el atributo action, le agregamos la ruta del controlador y modelo para ingresar
        $('#formulario').attr('action','<?= base_url('especialidadController/ingresar')?>');   
        });
...
$('#tabla_datos').on('click', '.edit', function(){
            ...
            $('#modalAgregar').modal('show');
            $('#modalAgregar').find('.modal-title').text('Editar especialidad');
            $('#formulario').attr('action', '<?= base_url('especialidadController/actualizar') ?>');
            $.ajax({
                type: 'ajax',
                method: 'post',
                url: '<?= base_url('especialidadController/get_datos') ?>',
                data: {id:id},
                dataType: 'json',
                success: function(datos){
                    $('#id').val(datos.id_especialidades);
                    $('#especialidad').val(datos.nombre_especialidad);
                    $('#tipo').val(datos.tipo_especialidad);
                }//fin de success
            });//fin de ajax
        });//fin del evento edit
...
$('#tabla_datos').on('click', '.borrar', function(){
			...
			$('#btnBorrar').unbind().click(function(){
				$.ajax({
					type: 'ajax',
					method: 'post',
					url: '<?= base_url('especialidadController/eliminar') ?>',
					data: {id:$id},
					dataType: 'json',
