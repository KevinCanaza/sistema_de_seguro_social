<script>
	...
	$(document).ready(function(){
		...
		function mostrarDatos(){
			$.ajax({
				type: 'ajax',
				url: '<?= base_url('enfermedadEstudianteController/get_enfermedad_estudiante') ?>',
				dataType: 'json',
				...
				success: function(datos){
					...
					for(i=0; i<datos.length; i++){
						...
						'<td>'+'<a href="javascript:;" class="edit" data="'+datos[i].id_enfer_est+'" > <button data-toggle="tooltip" title="Edit" class="pd-setting-ed"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a>  '+
						'<td>'+'<a href="javascript:;" class="borrar" data="'+datos[i].id_enfer_est+'" > <button data-toggle="tooltip" title="Trash" class="pd-setting-ed"><i class="fa fa-trash-o" aria-hidden="true" ></i></button></a>'+
						'</tr>';
						n++;
        ...
        $('#modalAgregar').find('.modal-title').text('Nueva enfermedad para el estudiante');
        //modificamos el atributo action, le agregamos la ruta del controlador y modelo para ingresar
        $('#formulario').attr('action','<?= base_url('enfermedadEstudianteController/ingresar')?>');   
        });
        ...
        function get_estudiante(){
            $.ajax({
                type: 'ajax',
                url: '<?= base_url('enfermedadEstudianteController/get_estudiante') ?>',
                dataType: 'json',
        ...
        get_enfermedad();//llamado a la funcion

        function get_enfermedad(){
            $.ajax({
                type: 'ajax',
                url: '<?= base_url('enfermedadEstudianteController/get_enfermedad') ?>',
                dataType: 'json',
                ...
                success: function(datos){
                    ...
                    opt +="<option value=''>--Seleccione una enfermedad--</option>";

                    for(i=0; i<datos.length; i++){
                        opt +="<option value='"+datos[i].id_enfermedad+"'>"+datos[i].nombre_enfermedad+"</option>";
                    }
                    $('#enfermedad').html(opt);
                }
            });
...
// Funcion ajax para hacer el proceso de edicion de datos
        $('#tabla_datos').on('click', '.edit', function(){
            ...
            $('#modalAgregar').modal('show');
            $('#modalAgregar').find('.modal-title').text('Editar enfermedad del estudiante');
            $('#formulario').attr('action', '<?= base_url('enfermedadEstudianteController/actualizar') ?>');
            $.ajax({
                type: 'ajax',
                method: 'post',
                url: '<?= base_url('enfermedadEstudianteController/get_datos') ?>',
                data: {id:id},
                dataType: 'json',
                success: function(datos){
                    $('#id').val(datos.id_enfer_est);
                    $('#estudiante').val(datos.id_estudiante);
                    $('#enfermedad').val(datos.id_enfermedad);
                }//fin de success
            });//fin de ajax
        });//fin del evento edit
        ...
        $('#tabla_datos').on('click', '.borrar', function(){
            ...
            $('#btnBorrar').unbind().click(function(){
                $.ajax({
                    type: 'ajax',
                    method: 'post',
                    url: '<?= base_url('enfermedadEstudianteController/eliminar') ?>',
                    data: {id:$id},
                    dataType: 'json',
                    success: function(respuesta){
