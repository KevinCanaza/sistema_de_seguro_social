<script>
	...
	$(document).ready(function(){
		...
		function mostrarDatos(){
			$.ajax({
				type: 'ajax',
				url: '<?= base_url('horarioMedicoController/get_horario') ?>',
				dataType: 'json',
				...
				success: function(datos){
					...
					for(i=0; i<datos.length; i++){
						...
						'<tr>'+
						'<td>'+n+'</td>'+
						'<td>'+datos[i].dia_laborable+'</td>'+
						'<td>'+datos[i].hora_inicio+'</td>'+ 
                        '<td>'+datos[i].hora_fin+'</td>'+
                        '<td>'+datos[i].cita_duracion+'</td>'+ 
						'<td>'+datos[i].nombre_doctor+'</td>'+
                        '<td> <button class="pd-setting">Active</button></td>'+
						'<td>'+'<a href="javascript:;" class="edit" data="'+datos[i].id_horario_medico+'" > <button data-toggle="tooltip" title="Edit" class="pd-setting-ed"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a>  '+
						'<td>'+'<a href="javascript:;" class="borrar" data="'+datos[i].id_horario_medico+'" > <button data-toggle="tooltip" title="Trash" class="pd-setting-ed"><i class="fa fa-trash-o" aria-hidden="true" ></i></button></a>'+
						'</tr>';
						n++;
        ...
        $('#modalAgregar').find('.modal-title').text('Nuevo horario');
        //modificamos el atributo action, le agregamos la ruta del controlador y modelo para ingresar
        $('#formulario').attr('action','<?= base_url('horarioMedicoController/ingresar')?>');   
        });
        ...
        function get_doctor(){
            $.ajax({
                type: 'ajax',
                url: '<?= base_url('horarioMedicoController/get_doctor') ?>',
                dataType: 'json',
        ...
        $('#tabla_datos').on('click', '.edit', function(){
            ...
            $('#modalAgregar').modal('show');
            $('#modalAgregar').find('.modal-title').text('Editar horario');
            $('#formulario').attr('action', '<?= base_url('horarioMedicoController/actualizar') ?>');
            $.ajax({
                type: 'ajax',
                method: 'post',
                url: '<?= base_url('horarioMedicoController/get_datos') ?>',
                data: {id:id},
                dataType: 'json',
                success: function(datos){
                    $('#id').val(datos.id_horario_medico);
                    $('#dia').val(datos.dia_laborable);
                    $('#entrada').val(datos.hora_inicio);
                    $('#salida').val(datos.hora_fin);
                    $('#duracion').val(datos.cita_duracion);
                    $('#doctor').val(datos.id_medico);
                }//fin de success
            });//fin de ajax
        });//fin del evento edit
        ...
        $('#tabla_datos').on('click', '.borrar', function(){
            ...
            $('#btnBorrar').unbind().click(function(){
                $.ajax({
                    type: 'ajax',
                    method: 'post',
                    url: '<?= base_url('horarioMedicoController/eliminar') ?>',
                    data: {id:$id},
                    dataType: 'json',
                    success: function(respuesta){
