<script>
	...
	$(document).ready(function(){
		...
		function mostrarDatos(){
			$.ajax({
				type: 'ajax',
				url: '<?= base_url('gradoParentescoController/get_grado_parentesco') ?>',
				dataType: 'json',
				...
				success: function(datos){
					...
					for(i=0; i<datos.length; i++){
						...
						'<tr>'+
						'<td>'+n+'</td>'+
                        '<td>'+datos[i].grado_parentesco+'</td>'+
						'<td>'+datos[i].nombre_estudiante+' '+datos[i].apellido_estudiante+'</td>'+
                        '<td>'+datos[i].nombre_apoderado+' '+datos[i].apellido_apoderado+'</td>'+
                        '<td> <button class="pd-setting">Active</button></td>'+
						'<td>'+'<a href="javascript:;" class="edit" data="'+datos[i].id_grado+'" > <button data-toggle="tooltip" title="Edit" class="pd-setting-ed"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a>  '+
						'<td>'+'<a href="javascript:;" class="borrar" data="'+datos[i].id_grado+'" > <button data-toggle="tooltip" title="Trash" class="pd-setting-ed"><i class="fa fa-trash-o" aria-hidden="true" ></i></button></a>'+
						'</tr>';
						n++;
        ...
        $('#modalAgregar').find('.modal-title').text('Nuevo apoderado del estudiante');
        //modificamos el atributo action, le agregamos la ruta del controlador y modelo para ingresar
        $('#formulario').attr('action','<?= base_url('gradoParentescoController/ingresar')?>');   
        });
        ...
        function get_estudiante(){
            $.ajax({
                type: 'ajax',
                url: '<?= base_url('gradoParentescoController/get_estudiante') ?>',
                dataType: 'json',
        ...
        get_apoderado();//llamado a la funcion

        function get_apoderado(){
            $.ajax({
                type: 'ajax',
                url: '<?= base_url('gradoParentescoController/get_apoderado') ?>',
                dataType: 'json',
                ...
                success: function(datos){
                    ...
                    opt +="<option value=''>--Seleccione un apoderado--</option>";

                    for(i=0; i<datos.length; i++){
                        opt +="<option value='"+datos[i].id_apoderado+"'>"+datos[i].first_name+" "+datos[i].last_name+"</option>";
                    }
                    $('#pariente').html(opt);
                }
            });
...
// Funcion ajax para hacer el proceso de edicion de datos
        $('#tabla_datos').on('click', '.edit', function(){
            ...
            $('#modalAgregar').modal('show');
            $('#modalAgregar').find('.modal-title').text('Editar apoderado del estudiante');
            $('#formulario').attr('action', '<?= base_url('gradoParentescoController/actualizar') ?>');
            $.ajax({
                type: 'ajax',
                method: 'post',
                url: '<?= base_url('gradoParentescoController/get_datos') ?>',
                data: {id:id},
                dataType: 'json',
                success: function(datos){
                    $('#id').val(datos.id_grado);
                    $('#grado').val(datos.grado_parentesco);
                    $('#estudiante').val(datos.id_estudiante);
                    $('#pariente').val(datos.id_apoderado);
        ...
        $('#tabla_datos').on('click', '.borrar', function(){
            ...
            $('#btnBorrar').unbind().click(function(){
                $.ajax({
                    type: 'ajax',
                    method: 'post',
                    url: '<?= base_url('gradoParentescoController/eliminar') ?>',
                    data: {id:$id},
                    dataType: 'json',
                    success: function(respuesta){
