<script>
	...
	$(document).ready(function(){
		...
		function mostrarDatos(){
			$.ajax({
				type: 'ajax',
				url: '<?= base_url('sintomaController/get_sintoma') ?>',
				dataType: 'json',
				...
				success: function(datos){
					...
					for(i=0; i<datos.length; i++){
						...
						'<td>'+'<a href="javascript:;" class="edit" data="'+datos[i].id_sintoma+'" > <button data-toggle="tooltip" title="Edit" class="pd-setting-ed"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a>  '+
						'<td>'+'<a href="javascript:;" class="borrar" data="'+datos[i].id_sintoma+'" > <button data-toggle="tooltip" title="Trash" class="pd-setting-ed"><i class="fa fa-trash-o" aria-hidden="true" ></i></button></a>'+
        ...
        $('#modalAgregar').find('.modal-title').text('Nueva sintoma');
        //modificamos el atributo action, le agregamos la ruta del controlador y modelo para ingresar
        $('#formulario').attr('action','<?= base_url('sintomaController/ingresar')?>');   
        });
...
$('#tabla_datos').on('click', '.edit', function(){
            ...
            $('#modalAgregar').modal('show');
            $('#modalAgregar').find('.modal-title').text('Editar sintoma');
            $('#formulario').attr('action', '<?= base_url('sintomaController/actualizar') ?>');
            $.ajax({
                type: 'ajax',
                method: 'post',
                url: '<?= base_url('sintomaController/get_datos') ?>',
                data: {id:id},
                dataType: 'json',
                success: function(datos){
                    $('#id').val(datos.id_sintoma);
                    $('#sintoma').val(datos.nombre_sintoma);
                }//fin de success
            });//fin de ajax
        });//fin del evento edit
...
$('#tabla_datos').on('click', '.borrar', function(){
			...
			$('#btnBorrar').unbind().click(function(){
				$.ajax({
					type: 'ajax',
					method: 'post',
					url: '<?= base_url('sintomaController/eliminar') ?>',
					data: {id:$id},
					dataType: 'json',
